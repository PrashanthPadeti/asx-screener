...
def compute_indicators(df: pd.DataFrame, shares: Optional[float]) -> pd.DataFrame:
    ...
    # Capped at 99.99 — values above that just mean "nearly all buying days"
    # and NUMERIC(10,6) on the hypertable only allows up to 9999.
...
def build_rows(asx_code: str, df: pd.DataFrame, since: Optional[date] = None) -> list:
    ...
    for _, row in df.iterrows():
        ...
        rows.append((
            ...
            g("up_down_vol_ratio_20d"), g("obv_rising"), g("volume_breakout"),
...
def main():
    ...
    _prove_envelope("technical_compute", conn)
    cur  = conn.cursor()
