...
const METRICS: Metric[] = [
  ...
  {
    id: 'eps_beat_rate_4q',
    name: 'EPS Beat Rate (4Q)',
    category: 'Quality',
    shortDesc: 'Fraction of the last 4 quarters where actual EPS beat the analyst estimate',
    definition: 'Measures how often a company has exceeded analyst EPS estimates over its 4 most recent reported quarters. A value of 1.0 means it beat every time; 0.0 means it missed every time. Derived from EODHD\'s quarterly EPS actual-vs-estimate history.',
    formula: 'EPS Beat Rate 4Q = (Quarters Beaten in last 4) ÷ 4',
    interpretation: 'A consistently high beat rate (0.75+) suggests management guides conservatively or the business has reliable, predictable earnings — both are positive quality signals. A low or declining beat rate can flag deteriorating forecasting accuracy or a business becoming harder to predict. Best read alongside eps_beat_rate_8q to separate a recent hot streak from a sustained pattern.',
    usedIn: ['Screener', 'Filter Builder', 'Query Mode'],
    tags: ['EPS', 'earnings surprise', 'beat', 'estimates', 'quality', 'consistency'],
  },
  {
    id: 'eps_beat_rate_8q',
    name: 'EPS Beat Rate (8Q)',
    category: 'Quality',
    shortDesc: 'Fraction of the last 8 quarters (2 years) where actual EPS beat the analyst estimate',
    definition: 'The same calculation as eps_beat_rate_4q but extended to the last 8 reported quarters, giving a longer-run view of earnings-surprise consistency that smooths out a single unusually good or bad year.',
    formula: 'EPS Beat Rate 8Q = (Quarters Beaten in last 8) ÷ 8',
    interpretation: 'Use this as the primary signal and eps_beat_rate_4q as a momentum check — a high 8Q rate with a 4Q rate that has dropped sharply may indicate the company is losing its forecasting edge or hitting a rough patch.',
    usedIn: ['Screener', 'Filter Builder', 'Query Mode'],
    tags: ['EPS', 'earnings surprise', 'beat', 'estimates', 'quality', 'consistency'],
  },
  {
    id: 'consecutive_eps_beats',
    name: 'Consecutive EPS Beats',
    category: 'Quality',
    shortDesc: 'Number of most-recent consecutive quarters where EPS beat the analyst estimate',
    definition: 'Counts the current unbroken streak of quarters (most recent first) where actual EPS exceeded the analyst estimate. The streak resets to 0 the moment a quarter misses or meets exactly.',
    formula: 'Consecutive EPS Beats = count of beats from the most recent quarter backwards, until the first non-beat',
    interpretation: 'A long, growing streak (6+ quarters) is a strong momentum and management-credibility signal — the market tends to reward companies that keep clearing the bar. A streak that just broke (drops to 0) after a long run is worth investigating, since it can mark an inflection point in the business.',
    usedIn: ['Screener', 'Filter Builder', 'Query Mode'],
    tags: ['EPS', 'earnings surprise', 'streak', 'momentum', 'quality'],
  },
  {
    id: 'analyst_upside_pct',
    name: 'Analyst Target Upside %',
    category: 'Quality',
    shortDesc: 'Percentage difference between the analyst consensus target price and the current share price',
    definition: 'Compares the Wall Street consensus 12-month target price (analyst_target_price) to the current closing price, expressed as a percentage. A positive value means analysts expect the stock to rise; a negative value means the stock is already trading above where analysts expect it to be.',
    formula: 'Analyst Upside % = (Analyst Target Price ÷ Current Price − 1) × 100',
    interpretation: 'High upside (20%+) combined with a strong analyst_consensus_score can flag undervalued opportunities, but treat target prices skeptically for stocks with low analyst_count — a single analyst\'s target carries much more noise than a 10-analyst consensus. Negative upside on a stock that has already rallied hard may indicate analysts haven\'t caught up with the move yet, not necessarily that it\'s overvalued.',
    usedIn: ['Screener', 'Filter Builder', 'Query Mode'],
    tags: ['analyst', 'target price', 'upside', 'valuation', 'consensus'],
  },
  {
    id: 'short_ratio',
    name: 'Short Ratio (Days to Cover)',
    category: 'Quality',
    shortDesc: 'EODHD days-to-cover — how many days of average trading volume it would take short sellers to close out their positions',
    definition: 'Calculated as shares currently sold short divided by the 50-day average daily trading volume. A high value means short positions are large relative to how much the stock normally trades, so unwinding them would take many trading days.',
    formula: 'Short Ratio = Shares Short ÷ 50-Day Average Daily Volume',
    interpretation: 'A high short ratio (5+ days) combined with positive news can trigger a short squeeze, since shorts struggle to exit quickly without driving the price up further. This is EODHD\'s own short-interest snapshot — for ASX-specific regulatory short positions, prefer short_pct and short_interest_chg_1w, which are sourced from ASIC\'s daily short-position reports and are more authoritative for Australian stocks.',
    usedIn: ['Screener', 'Filter Builder', 'Query Mode'],
    tags: ['short interest', 'days to cover', 'short squeeze', 'EODHD'],
  },
  {
    id: 'years_listed',
    name: 'Years Listed (IPO Age)',
    category: 'Quality',
    shortDesc: 'Number of years since the company\'s IPO date — a proxy for business maturity and track record',
    definition: 'The time elapsed since the company first listed, calculated from its IPO date to today. Younger listings (under 3 years) generally have less operating history to evaluate, while older listings have survived more economic cycles.',
    formula: 'Years Listed = (Today − IPO Date) ÷ 365.25',
    interpretation: 'Useful as a screening filter to exclude very recent IPOs when looking for proven track records (e.g., years_listed > 5), or conversely to specifically find newly listed companies. Not a quality signal on its own — a 20-year-old company can still be poorly run, and a 2-year-old company can be excellent — but it contextualises how much history backs up the other metrics in this screener.',
    usedIn: ['Screener', 'Filter Builder', 'Query Mode'],
    tags: ['IPO', 'company age', 'maturity', 'listing history'],
  },

  // ── TIER-2: ASX ANNOUNCEMENTS SIGNALS ────────────────────────────────────
  {
    id: 'capital_raise_count_1y',
    name: 'Capital Raises (1Y)',
    category: 'Quality',
    shortDesc: 'Number of placement, rights issue, entitlement offer, or SPP announcements in the trailing 12 months',
    definition: 'Counts ASX announcements classified as capital-raising activity (placements, rights issues, entitlement offers, share purchase plans) filed by the company over the last 12 months. Sourced from the free public ASX announcements API.',
    formula: 'Capital Raise Count 1Y = count of capital-raising announcements, trailing 12 months',
    interpretation: 'Frequent capital raises (2+ per year) often signal a company that cannot fund itself from operating cash flow and is diluting existing shareholders to stay afloat — common among speculative miners and pre-revenue biotechs. Zero raises over a long period is generally a positive sign of self-sufficiency, though large, well-capitalised companies occasionally raise opportunistically for acquisitions without it being a red flag.',
    usedIn: ['Screener', 'Filter Builder', 'Query Mode'],
    tags: ['capital raise', 'dilution', 'placement', 'rights issue', 'governance', 'ASX announcements'],
  },
  {
    id: 'recent_capital_raise',
    name: 'Recent Capital Raise (90d)',
    category: 'Quality',
    shortDesc: 'True if the company has announced a capital raise in the last 90 days',
    definition: 'A boolean flag that is true when a placement, rights issue, entitlement offer, or SPP has been announced within the most recent 90 days. This isolates acute, near-term dilution risk from the longer-run capital_raise_count_1y trend.',
    formula: 'Recent Capital Raise = TRUE if any capital-raising announcement in the last 90 days',
    interpretation: 'A fresh capital raise typically means new shares are about to hit the market (dilution) and the raise price often anchors the stock in the short term. Useful as an exclusion filter for investors wary of near-term dilution, or as a discovery filter for investors specifically looking for post-raise entry points once the new capital is deployed.',
    usedIn: ['Screener', 'Filter Builder', 'Query Mode'],
    tags: ['capital raise', 'dilution', 'placement', 'recent', 'ASX announcements'],
  },
  {
    id: 'trading_halt_count_1y',
    name: 'Trading Halts (1Y)',
    category: 'Quality',
    shortDesc: 'Number of trading halt announcements in the trailing 12 months',
    definition: 'Counts ASX trading halt announcements filed by the company over the last 12 months. Trading halts are typically requested ahead of material announcements (capital raises, major contracts, regulatory news) to ensure an orderly market.',
    formula: 'Trading Halt Count 1Y = count of trading-halt announcements, trailing 12 months',
    interpretation: 'A high frequency of trading halts (3+ per year) can indicate a company that is frequently in the middle of material corporate activity — which may be positive (an active growth phase) or negative (repeated capital raises or crisis management). Compare against capital_raise_count_1y, since many halts precede a raise.',
    usedIn: ['Screener', 'Filter Builder', 'Query Mode'],
    tags: ['trading halt', 'volatility', 'corporate activity', 'ASX announcements'],
  },
  {
    id: 'director_change_count_1y',
    name: 'Director Changes (1Y)',
    category: 'Quality',
    shortDesc: 'Number of director or substantial-holder-change announcements in the trailing 12 months',
    definition: 'Counts ASX announcements relating to changes in directors or substantial shareholders (Appendix 3X/3Y filings, Form 604/605 notices) over the last 12 months. Captures both board turnover and significant changes in major shareholdings.',
    formula: 'Director Change Count 1Y = count of director/substantial-holder-change announcements, trailing 12 months',
    interpretation: 'A high rate of director turnover can signal governance instability or a board in transition following a strategic shift. A flurry of substantial-holder notices can indicate accumulation or distribution by large investors — worth cross-referencing with percent_institutions and percent_insiders to see whether ownership concentration is rising or falling.',
    usedIn: ['Screener', 'Filter Builder', 'Query Mode'],
    tags: ['director', 'substantial holder', 'governance', 'board turnover', 'ASX announcements'],
  },
  {
    id: 'days_since_last_announcement',
    name: 'Days Since Last Announcement',
    category: 'Quality',
    shortDesc: 'Number of days since the company\'s most recent ASX disclosure of any type',
    definition: 'The number of days elapsed since the company\'s last announcement to the ASX, regardless of document type. A low number means the company is actively disclosing; a high number can indicate a quiet period or, for very high values, a dormant or inactive disclosure history.',
    formula: 'Days Since Last Announcement = Today − Date of Most Recent Announcement',
    interpretation: 'Most active ASX-listed companies disclose at least quarterly (required for many), so values consistently above ~120 days can flag shell companies, suspended stocks, or businesses that have gone unusually quiet — worth investigating before assuming it is simply a calm period.',
    usedIn: ['Screener', 'Filter Builder', 'Query Mode'],
    tags: ['disclosure', 'announcement', 'recency', 'dormant', 'ASX announcements'],
  },
  {
    id: 'announcement_count_1y',
    name: 'Announcement Count (1Y)',
    category: 'Quality',
    shortDesc: 'Total number of ASX announcements of any type filed in the trailing 12 months',
    definition: 'A simple count of all ASX disclosures (filings, reports, news, corporate actions) made by the company over the last 12 months, regardless of category. Serves as a baseline measure of corporate activity and disclosure engagement.',
    formula: 'Announcement Count 1Y = count of all announcements, trailing 12 months',
    interpretation: 'Very low counts can indicate either an unusually quiet, stable business or a disengaged discloser. Very high counts often correlate with active corporate activity (M&A, capital raises, exploration updates for miners) — read this alongside the more specific counts above (capital raises, trading halts, director changes) to understand what is actually driving the volume.',
    usedIn: ['Screener', 'Filter Builder', 'Query Mode'],
    tags: ['disclosure', 'announcement', 'corporate activity', 'ASX announcements'],
  },
